...
# async def test_controller_create_should_fail_insertion(client, products_url):

#     invalid_product = {
#         "id": "2be5becc-73d4-4059-894a-466aad2ff36f",
#         "name": "Iphone 15 Pro Max",
#         "quantity": 10,
#         "price": 10.200,
#         "status": True,
...
async def test_controller_get_query_should_return_success(client, products_url):
...
async def test_controller_patch_should_return_success(
    ...
    response = await client.patch(
        f"{products_url}{product_inserted.id}", json={"price": "10.500"}
    )
...
async def test_controller_patch_should_return_not_found(client, products_url):
    invalid_id = uuid4()
    response = await client.patch(
        f"{products_url}{invalid_id}", json={"price": "10.500"}
    )

    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {"detail": f"Product not found with filter: {invalid_id}"}
